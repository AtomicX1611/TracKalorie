import { MealModel } from '../meals/meals.schema';
import { GoalModel } from '../goals/goals.schema';
...
export const nutritionRepository = {
  async getWeeklyTrend(
    ...
    to: Date,
    timezone: string
  ): Promise<Array<{ day: string; calories: number; proteinG: number; carbG: number; fatG: number }>> {
  ...
  async getGoalVsActual(
    ...
    day: string;
    actual: { calories: number; proteinG: number; carbG: number; fatG: number };
    goal: { dailyCalorieTarget: number; proteinTargetG: number; carbTargetG: number; fatTargetG: number } | null;
  }>> {
